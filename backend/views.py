...
class home(View):
     ...
     def post(self, request): 
            ...
            d11= float(request.POST.get("eleven"))
            d12= float(request.POST.get("twelve"))
            d13= float(request.POST.get("thirteen"))
            d2 = 1 if d2.lower() == "male" else 0
...
# Train model
            user_input_df = pd.DataFrame([[d1, d2, d3, d4, d5, d6, d7, d8, d9, d10,d11,d12,d13]], columns=feature_names)
            lr = LinearRegression()
...
# Predict 
            ...
            p = "Yes" if prediction[0] >= 0.5 else "No"
